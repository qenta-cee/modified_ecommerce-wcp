...
class wcp_invoice extends WirecardCheckoutPagePayment
{
    ...
    function selection()
    {
        ...
        if ($this->getConfigParam('PROVIDER') == 'payolution' && $this->getConfigParam('PAYOLUTION_TERMS') == 'True') {
            ...
            $content['fields'][] = array(
                'title' => sprintf('<input id="%s" type="checkbox" name="consent" autocomplete="off" class="wcp_invoice consent"/>',
                    $fieldId),
        ...
        if (!$this->_b2b) {
            ...
            $field = "<select style='width:auto;' name='wcp_invoice_birthday_day' class='mandatory'>$days</select>";

            $field .= "<select style='width:auto;' name='wcp_invoice_birthday_month' class='mandatory'>$months</select>";

            $field .= "<select style='width:auto;' name='wcp_invoice_birthday_year' class='mandatory'>$years</select>";
        ...
        $(function () {
            ...
            $.fn.wcpValidateInvoice = function (messageBox) {
 ...
 * @return string
 */
function wcp_invoice_cfg_pull_down_provider($provider, $key = '')
{
